...
class AtomicFactGenerator(object):
  ...
  async def run(self, generation: str, cost_estimate: Optional[bool] = None):
    """Convert the generation into a set of atomic facts."""
    assert isinstance(generation, str), 'generation must be a string'
    paragraphs = [
        para.strip() for para in generation.split('\n') if para.strip()
    ]
    return await self.get_atomic_facts_from_paragraph(
        paragraphs, cost_estimate=cost_estimate
    )

  async def get_atomic_facts_from_paragraph(self, paragraphs, cost_estimate=None):
    """Get the atomic facts from the paragraphs."""
    sentences, para_breaks = [], []

    # Process paragraphs concurrently
    async def process_paragraph(para_idx, paragraph):
      if para_idx > 0:
    ...
    processed_sentences = await asyncio.gather(*[
      process_paragraph(para_idx, paragraph) 
      for para_idx, paragraph in enumerate(paragraphs)
    ])

    # Combine processed sentences
    sentences = [sent for para_sents in processed_sentences for sent in para_sents]
    ...
    atoms_or_estimate = await self.get_init_atomic_facts_from_sentence(
        filtered_sentences,
    ...
    # Process atomic facts pairs concurrently  
    async def process_sentence(i, sent):
      if not self.is_bio and (
        ...
        return (sent, atoms[sent])

    atomic_facts_pairs = await asyncio.gather(*[
      process_sentence(i, sent) for i, sent in enumerate(sentences)
    ])

    # Handle bio-specific postprocessing
    if self.is_bio:
  ...
  async def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
    ...
    if cost_estimate:
      ...
      async def process_prompt(prompt):
        ...
        system_message = ""
        response, prompt_tok_cnt, response_tok_cnt = await self.get_model_response.async_get_response(system_message, prompt_to_send, cost_estimate_only=cost_estimate)
        ...
        return prompt_to_sent[prompt], sentences_from_output

      # Process prompts concurrently
      results = await asyncio.gather(*[process_prompt(prompt) for prompt in prompts])
      
      for sentence, facts in results:
